function [est, M, history] = subject_mle_pss(f, model, modeli, narm, nctx, ntrials, sd_r, noise_r_i, i, true_params, maxiters)
    %SUBJECT_MLE_PSS optimizes parameters for a single subject
    % by restarting the provided optimizer until there is no further
    % improvement in likelihood.  Uses pattern search with smooth constraints.
    ...
    while improved == 1 && k < maxiters
        ...
        [xmin,fmin,flg,out] = patternsearch(f,x0,[],[],[],[],[],[],[],psopt);  % Optimize
