function [est, M, history] = subject_mle_fminb(f, model, LB, UB, modeli, narm, nctx, ntrials, sd_r, noise_r_i, i, true_params, maxiters)
    %SUBJECT_MLE_FMINB optimizes parameters for a single subject
    % by restarting the provided optimizer until there is no further
    % improvement in likelihood.  Uses fmincon with bound constraints.
    ...
    while improved == 1 && k < maxiters
        ...
        [xmin,fmin,flg,out] = fmincon(f,x0,[],[],[],[],LB,UB,[],fminopt); % Optimize
